...
const { getTemplateContext, getTemplates, populateDatabase } = require('./actions/populateDatabase')
...
function loadDynamoEndpoint(env, dynamoConfig) {
  if(env.LOCAL_ENDPOINT_ENABLED != null) {
    dynamoConfig.endpoint = 'http://localhost:8000';
    dynamoConfig.accessKeyId = 'key';
    dynamoConfig.secretAccessKey = 'secret';
  }
  if (typeof env.DYNAMO_ENDPOINT === 'string') {
    ...
    console.log(
      clc.yellow(
        '  DYNAMO_ENDPOINT is not defined.'
      )
    )
  }
}
...
exports.createServer = (dynamodb, docClient, expressInstance = express()) => {
  ...
  app.get('/usecase-templates', asyncMiddleware((req, res) => {
    const data = {
      templates: getTemplates()
    }
    res.render('usecase-template', data)
  }))

  app.get('/usecase-templates/:templateId', asyncMiddleware((req, res) => {
    const templateId = req.params.templateId;
    const templateContext = getTemplateContext(templateId);
    const data = {
      template: templateContext
    }
    res.render('usecase-template-item', data)
  }))

  app.post('/usecase-templates/populate', asyncMiddleware(async (req, res) => {
    const templateId = req.body.id;
    const templateContext = req.body.context;
    res.json(await populateDatabase(templateId, templateContext));
  }))
